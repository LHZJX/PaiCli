...
- 📦 沙箱根目录（默认 CLI 启动目录）：目录内的文件操作自动放行
- 🛡️ 越界文件操作（读/写/列出沙箱外的路径）与无法判定安全的 Shell 命令，会先征求你的同意：
  `y` 仅同意这一次 / `a` 本会话同类操作不再询问 / 回车拒绝（拒绝结果会反馈给 LLM）
- ✅ 只读检查命令白名单（`ls`/`pwd`/`echo`/`git status` 等）自动放行
- ⏱️ 命令执行带超时强杀与输出上限，防止死循环拖垮 CLI

| 环境变量 | 作用 | 默认值 |
|---|---|---|
| `PAI_SANDBOX_DIR` | 沙箱根目录 | CLI 启动目录 |
| `PAI_CMD_TIMEOUT_SEC` | execute_command 超时（秒），超时强制终止 | 600 |
| `PAI_CMD_OUTPUT_LIMIT` | execute_command 输出上限（字节），超出截断 | 524288 |
